...
const InterviewerDashboard: React.FC<InterviewerDashboardProps> = ({ appState }) => {
  ...
  const SortButton: React.FC<{ sortKey: SortKey, label: string }> = ({ sortKey, label }) => (
    <button
      ...
      className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${activeSort === sortKey ? 'bg-[#4285F4] text-white' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}
    >
      {label}
    </button>
  );
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
                <div className="relative w-full sm:w-auto sm:flex-1">
                    ...
                    <input
                        ...
                        className="w-full p-2 pl-10 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#4285F4] focus:outline-none"
                    />
                </div>
                ...
                <table className="w-full text-left table-auto">
                    ...
                    <tbody>
                        ...
                        >
                            ...
                            <td className="p-3">
                                <button onClick={() => setSelectedCandidate(candidate)} className="text-[#4285F4] hover:text-[#357ABD] font-semibold">View Details</button>
                            </td>
                        </tr>
                        ))}
                    </tbody>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-700">
                <strong>Score:</strong> <span className="font-bold text-[#4285F4]">{selectedCandidate.finalScore}%</span>
              </p>
                ...
                {selectedCandidate.questions.map((q, index) => (
                  <div key={q.id} className="p-4 bg-white border border-gray-200 rounded-lg shadow-sm">
                    <div className="flex items-start gap-3">
                      <BotIcon className="w-6 h-6 p-1 bg-[#4285F4] text-white rounded-full flex-shrink-0 mt-1" />
                      <div>
